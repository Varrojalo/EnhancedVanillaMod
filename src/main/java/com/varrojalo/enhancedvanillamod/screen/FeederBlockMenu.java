...
public class FeederBlockMenu extends AbstractContainerMenu {

    public final FeederBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;

    public FeederBlockMenu(int pContainerId, Inventory pInventory, BlockPos pBlockPos){
        this(pContainerId, pInventory,pInventory.player.level().getBlockEntity(pBlockPos),new SimpleContainerData(1));
    }
    public FeederBlockMenu(int pContainerId, Inventory pInventory, FriendlyByteBuf pExtraData)
    {
        this(pContainerId, pInventory, pInventory.player.level().getBlockEntity(pExtraData.readBlockPos()),
                new SimpleContainerData(1));
    }

    public FeederBlockMenu(int pContainerId, Inventory pInventory, BlockEntity pEntity, ContainerData pData) {
        super(ModMenuTypes.FEEDER_MENU.get(), pContainerId);
        checkContainerSize(pInventory,1);
        blockEntity = ((FeederBlockEntity)pEntity);
